...
const { redisLogger } = require('../utils/logger'); // 🔔 Redis 전용 로거 사용
...
async function checkRedisStatus() {
  try {
    ...
    console.log('Redis Status:', info);
    redisLogger.info(`Redis 상태 확인 완료`, {
      version: info.redis_version,
      mode: info.redis_mode
    });
  } catch (err) {
    console.error('Redis Status Check Error:', err);
    redisLogger.error('Redis 상태 점검 실패', {
      message: err.message,
      stack: err.stack
    });
  }
...
redis.on('connect', () => {
  ...
  redisLogger.info('✅ Redis 서버 연결 성공');
  redisStatusGauge.set(1);
});

redis.on('ready', () => {
  console.log('Redis ready');
  redisLogger.info('🔄 Redis 클라이언트 준비 완료');
  checkRedisStatus();
});

redis.on('reconnecting', () => {
  console.log('Redis reconnecting');
  redisLogger.warn('⚠️ Redis 재연결 시도 중...');
  redisStatusGauge.set(0);
});

redis.on('end', () => {
  console.log('Redis connection ended');
  redisLogger.warn('🛑 Redis 연결 종료됨');
  redisStatusGauge.set(0);
});

redis.on('error', (err) => {
  console.error('Redis Error:', err);
  redisLogger.error('❌ Redis 에러 발생', {
    message: err.message,
    code: err.code,
    stack: err.stack
  });
...
async function shutdown() {
  redisLogger.info('🧹 Redis 종료 시도 중...');
  try {
    await redis.quit();
    redisLogger.info('✅ Redis 안전하게 종료됨');
  } catch (err) {
    redisLogger.error('Redis 종료 중 에러 발생', {
      message: err.message,
      stack: err.stack
    });
  } finally {
